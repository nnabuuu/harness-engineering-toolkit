...
| 工具 | 功能 | 适用对象 |
|------|------|---------|
| **`/harness-retro`** | 对已完成的 harness 任务进行复盘。分析运行数据（分数、评估报告、变更日志），覆盖 6 个维度：收敛性、瓶颈维度、重复劳动、成本效率、提示词质量、跨任务模式。生成具体的提示词/评分标准改进建议。两个入口：完整复盘（分析 → 改进建议 → 可选归档）或独立归档（跳过分析，直接将已完成任务移至 `_archive/`，保留 DAGU 执行历史）。 | 运行过 harness 的人 |

### 维护

| 工具 | 功能 | 适用对象 |
|------|------|---------|
| **`/harness-skill-prune`** | 模型/平台升级后审计已安装的 skill。先分类每个 skill 在补偿什么（模型能力、平台能力、私有知识、强制执行），再决定淘汰、精简还是保留。先出报告，确认后才动文件。淘汰的 skill 移入 `graveyard/` 并附墓志铭，所有操作可回滚。 | skill 越装越多的人 |
