...
| 工具 | 功能 | 适用对象 |
|------|------|---------|
| **`/harness-self-check`** | 交互式诊断。假设你已经有了 harness，然后通过追问暴露你以为有但其实没有的部分。五种自欺，逐个探测。 | 任何行业 |
| **`/harness-audit`** | 自动化审计。扫描代码仓库中的文件、CI 配置和 git 历史，检测五种自欺，发现能自动修的问题会问你要不要当场修掉。 | 软件开发者 |

### 构建（即将推出）

| 工具 | 功能 | 适用对象 |
|------|------|---------|
| **`/harness-plan`** | 为具体任务或项目设计 harness 规格——确定目标、所需上下文、检查规则、评估标准。 | 任何行业 |
| **`/harness-build`** | 根据 plan 生成 harness 产物——指令文件、检查脚本、文档模板。 | 开发者 |
...
**单次任务** — 让一个 agent 把一件事做对。目标清晰、上下文到位、检查在交付前运行、产出被评估。自欺 1-3 主要在这个尺度。

**长期运转** — 让系统在数周、数月后仍然正确。目标保持最新、上下文不腐烂、检查规则跟上新的错误模式、飞轮持续转动。自欺 5 在这个尺度。自欺 4 横跨两者。

当前工具覆盖两个尺度。未来的 `/harness-plan` 和 `/harness-build` 将包含长期 harness 设计（启动仪式、进度文件、定期巡检、harness 版本化）。
...
**软件开发者想要自动扫描？** 在仓库里跑 `/harness-audit`，检测自欺，提供修复。
